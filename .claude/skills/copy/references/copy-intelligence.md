# Copy Intelligence — Biblioteca Viva de Copy Escalada

> "Swipe não é copiar. É entender por que funcionou — e aplicar o princípio, não o texto." — Regra da Inteligência Competitiva

---

## Como Usar Este Arquivo

1. **Monitoramento semanal:** 15 min na Biblioteca de Anúncios Meta → registrar novos swipes
2. **Antes de escrever:** Verificar se existe ângulo mapeado para o nicho/produto
3. **Quando um concorrente escala:** Registrar o swipe card completo → extrair o princípio → adaptar o eixo, nunca o texto
4. **Atualização:** Adicionar swipes novos no template abaixo. Marcar os que saíram de ar com ~~riscado~~.

---

## CAMADA 1 — Copywriters BR Ativos

### Diogo Ramalho
**Nicho principal:** Infoprodutos (marketing digital, negócios online)
**Mecanismo favorito:** Puzzle Pieces — dividir a transformação em "peças" que só fazem sentido juntas
**Ângulo mais usado:** "O sistema inteiro que ninguém mostra" — posiciona o produto como a peça que estava faltando
**Formato preferido:** VSL longa (30-45 min) + carta de vendas longa
**O que aprender:**
- Como construir percepção de completude antes de revelar o produto
- MUP/MUS integrados de forma educativa (o prospect aprende algo real antes do pitch)
- Sequência de emails de pré-lançamento com valor genuíno

**Como monitorar:** Ads ativos nas páginas dos produtos dele (Biblioteca Meta) + grupo de alunos no Facebook

---

### Evaldo Albuquerque
**Nicho principal:** Relacionamento, liberdade emocional, autoconhecimento
**Mecanismo favorito:** 16-Word Belief — toda a copy serve uma crença central de 16 palavras
**Ângulo mais usado:** Identidade + transformação interna → resultados externos
**Formato preferido:** Carta de vendas + VSL emocional
**O que aprender:**
- Construção da One Belief antes de escrever uma palavra
- Escalada emocional até nível 4-5 antes de revelar mecanismo
- Absolvição profunda (não superficial) — o prospect precisa realmente acreditar que não é culpa dele

**Como monitorar:** Biblioteca de Anúncios + páginas de produto publicadas

---

### Max Peters
**Nicho principal:** Copywriting, marketing direto, negócios
**Mecanismo favorito:** NUUPPECC — scoring de headlines por 8 dimensões
**Ângulo mais usado:** Educação sobre copy como produto de transformação de negócios
**Formato preferido:** Email DR + VSL + conteúdo educativo no Instagram
**O que aprender:**
- Técnica de headline scoring (usar como filtro antes de publicar qualquer headline)
- Estrutura de email DR compacto (150-300 palavras com impacto máximo)
- Como usar especificidade para destruir genericidade

**Como monitorar:** Lista de email (se inscrever como lead) + perfil no Instagram

---

### Vitor Mound
**Nicho principal:** Paid social, criativos de vídeo
**Mecanismo favorito:** Hook Anatomy — [Qualificação] + [Emoção] + [Curiosidade]
**Ângulo mais usado:** Hook de identidade + inversão de crença comum
**Formato preferido:** Vídeo curto (15-30s) + hook de 5 segundos
**O que aprender:**
- Como construir hooks que qualificam E despertam emoção simultaneamente
- Dopamine dispatches — mini-hooks a cada 3-4 parágrafos para manter atenção
- Diferença entre hook de feed (scroll stop) e hook de VSL (engajamento sustentado)

**Como monitorar:** Perfil no Instagram + anúncios ativos de clientes dele

---

### Caio Carneiro
**Nicho principal:** Tráfego pago, Meta Ads
**Mecanismo favorito:** Creative testing sistemático — nunca um winner, sempre um benchmark
**Ângulo mais usado:** Performance + dados como copy (prova social de resultado)
**Formato preferido:** Vídeo curto UGC (user-generated content style)
**O que aprender:**
- Como criar copy que parece UGC (alta credibilidade, baixo CPM)
- Estrutura de testes de creative que gera dados acionáveis
- Quando parar de testar e escalar

**Como monitorar:** Canal no YouTube + Biblioteca de Anúncios (clientes)

---

### Davi — Low Ticket Makers
**Nicho principal:** Low Ticket (infoprodutos de entrada, nutraceuticos)
**Mecanismo favorito:** One Pager + pesquisa sistemática de mercado antes de qualquer copy
**Ângulo mais usado:** Mecanismo de nicho descoberto via volume de pesquisa → Big Idea como 80/20 da oferta
**Formato preferido:** Funil de Low Ticket com quiz + oferta de mecanismo nomeado
**O que aprender:**
- **One Pager como base de trabalho:** uma página com pesquisa de público, ideias de mecanismo em ordem de prioridade e funis de concorrentes mapeados — tudo antes de escrever uma linha
- **Processo de pesquisa em 2 dias:** Claude → 10 palavras-chave do nicho → Biblioteca de Anúncios filtrado por últimos 10 dias + vídeo → planilha com centenas de funis → ideias de mecanismo em sequência de prioridade
- **Copy = extensão de pesquisa (não escrita):** pesquisa de público + mecanismo + mercado (concorrentes, YouTube, EUA, SwipeFiles) → cria a Big Idea → Big Idea é o 80/20 de tudo
- **Volume acelera validação:** 8 Low Tickets em 30 dias aumenta probabilidade de acertar um mecanismo → volume > qualidade quando se está recomeçando
- **Gary Bencivenga aplicado:** 50% do tempo de criação = pesquisa. Copy ruim + Big Idea boa vende. Copy boa + Big Idea ruim não vende.

**Processo One Pager (passo a passo):**
1. Pede ao Claude as 10 palavras-chave mais pesquisadas do nicho
2. 2 dias inteiros na Biblioteca de Anúncios — filtro: últimos 10 dias + mais veiculação + vídeo
3. Coloca todos os funis encontrados numa planilha
4. Das centenas de funis → extrai ideias de mecanismo
5. Ordena por prioridade: mais concorrentes + mais anunciado + mais sentido para o avatar
6. Escolhe mecanismo #1 → cria oferta

**Como monitorar:** Comunidade Low Ticket Makers + conteúdo orgânico

**Fonte:** `mentoria/davi-low-ticket-5-dias-2026-04-10.md`

---

### Luca + Lucas Collin (Copy Chief BLACK)
**Nicho principal:** Copy de alta performance, nutraceuticos, infoprodutos premium
**Mecanismo favorito:** 5 Lentes BLACK (Cena de Filme, Emoção Coerente, Logo Test, Visceral, Zero Hedges)
**Ângulo mais usado:** Especificidade extrema + VOC como copy (frases exatas do prospect)
**Formato preferido:** VSL + carta de vendas longa com proof density alta
**O que aprender:**
- Logo Test como filtro obrigatório (se o concorrente pode usar sem mudar, refazer)
- Anti-homogenização ativa — não apenas evitar o genérico, mas buscar o inesperadamente específico
- Proof density em mercados céticos (1 prova por 1 claim, não 1 por 4)

**Como monitorar:** Conteúdo público + swipes que circulam em grupos de copy BR

---

## CAMADA 2 — Swipes de Concorrentes Diretos

### Template de Swipe Card
```
---
SWIPE: [Nome descritivo]
Produto: [nome do produto]
Nicho: [emagrecimento / nutra / infoproduto / etc]
Data de entrada no ar: [mês/ano estimado]
Status: [ATIVO / INATIVO]

AVATAR: [descrição do avatar alvo]
AWARENESS LEVEL: [1-5]
DRE: [emoção dominante — medo / vergonha / raiva / frustração / desejo]

HOOK/ABERTURA: 
"[primeiras linhas exatas ou paráfrase fiel]"

MUP: [nome do mecanismo do problema]
MUS: [nome do mecanismo da solução]
ÂNGULO: [o que é único nesse ângulo]

O QUE ESTÁ FUNCIONANDO: [hipótese — por que está escalando]
O QUE ADAPTAR: [o eixo/princípio a aplicar — não o texto]
---
```

---

---

## TOP 20 — American Swipe (mapeado em 2026-04-13)

> Fonte: American Swipe dashboard "Escalando agora" — ordenado por volume de anúncios ativos.

### Ranking Completo

| # | Produto | Nicho | Anúncios | Desde | Idioma | Formato |
|---|---|---|---|---|---|---|
| 1 | Celtic Salt Trick - Vigor Peak | Disfunção Erétil | 50.001 | jan/26 | Deutsch | VSL/Nutra |
| 2 | Truque com Raiz Vermelha | Visão | 50.001 | mai/25 | English | VSL/Nutra |
| 3 | Reversal Ritual - Glyco Lean | Diabetes | 33.112 | mar/26 | English | VSL/Nutra |
| 4 | Truque da Gelatina Rosa - Gelatide | Emagrecimento | 17.880 | dez/25 | English | VSL/Nutra |
| 5 | The Honey Trick - Neurozen | Alzheimer | 16.532 | jan/26 | English | VSL/Nutra |
| 6 | Pink Salt Trick - Neuro Salt | Neuropatia | 16.321 | mar/26 | English | VSL/Nutra |
| 7 | The Gelatin Trick - Mounja Gummy | Emagrecimento | 13.743 | jan/26 | English | VSL/Nutra |
| 8 | Cholesterol Brain Hack - Primal Brain | Alzheimer | 12.520 | mar/26 | English | VSL/Nutra |
| 9 | Café Bariátrico | Emagrecimento | — | jan/26 | PT-BR | YouTube/Low Ticket |
| 10 | Escudo de Microplástico | Fungos | 9.565 | jan/26 | English | VSL/Nutra |
| 11 | Truque da Gelatina - BurnSlim | Emagrecimento | 9.532 | jan/26 | English | VSL/Nutra |
| 12 | GLP-1 Natural com Gelatina - LipoMax | Emagrecimento | 8.406 | abr/26 | English | VSL/Nutra |
| 13 | Truque Simples com Gelatina | Emagrecimento | 7.834 | out/25 | English | VSL/Nutra |
| 14 | Truque do Sal Rosa - OzemFit | Emagrecimento | 7.756 | nov/25 | English | VSL/Nutra |
| 15 | Farmacopéia Temperista | Outros | 7.407 | jun/25 | PT-BR | Quiz/Low Ticket |
| 16 | Protocolo Gut-Spine – Back Biome | Outros | 7.331 | fev/26 | English | VSL/Nutra |
| 17 | Glucose Reset Ritual - Gluco Zen | Diabetes | 6.234 | fev/26 | English | VSL/Nutra |
| 18 | Casca de Maçã | Diabetes | 6.221 | jan/26 | English | VSL/Infoproduto |
| 19 | Glucose Reset Ritual - GL Pro | Diabetes | 5.177 | nov/25 | English | VSL/Nutra |
| 20 | Natural Honey Trick | Diabetes | 4.080 | abr/26 | English | VSL/Nutra |

---

### Padrões de Mecanismo Identificados

**Padrão 1 — "[Ingrediente Cotidiano] + Trick"** (dominante — 8+ offers)
`Celtic Salt Trick` / `Truque com Raiz Vermelha` / `Honey Trick` / `Pink Salt Trick` / `Gelatin Trick` / `Truque da Gelatina`
- MUS gimmick name: "[Ingrediente]" + "Trick/Truque" = ingrediente acessível como solução inesperada
- Eixo: Inversão de crença ("o que parece comum esconde o segredo") + Absolvição ("você não sabia desse truque")
- **Adaptar:** "[Ingrediente/componente] + Truque/Protocolo/Ritual" como gimmick name para BioRitmo

**Padrão 2 — "Reset Ritual"** (diabetes — 3 offers)
`Reversal Ritual - Glyco Lean` / `Glucose Reset Ritual - Gluco Zen` / `Glucose Reset Ritual - GL Pro`
- MUS: "Ritual" como ação do mecanismo (implica processo específico, não produto genérico)
- Eixo: Reversão do problema ("reverter o dano") vs. apenas controlar
- **Adaptar:** "[Resultado desejado] + Reset/Reversal + [Nome do produto]" para produtos com ângulo de reversão

**Padrão 3 — "GLP-1 Natural"** (emagrecimento — emergindo)
`GLP-1 Natural com Gelatina - LipoMax` / `Truque do Sal Rosa - OzemFit`
- MUS: conecta ao Ozempic (farmacêutico famoso) mas como versão natural/sem prescrição
- Eixo: "O que o remédio caro faz, este ingrediente faz naturalmente"
- **Adaptar:** Conectar BioRitmo a resultados conhecidos de medicamentos (sem mencionar) via mecanismo natural

**Padrão 4 — "Hack/Protocol"** (cognição/neurológico)
`Cholesterol Brain Hack - Primal Brain` / `Protocolo Gut-Spine – Back Biome`
- MUS: "Hack" = atalho inesperado via mecanismo cerebral/biológico
- Eixo: Conexão sistêmica (gut-brain, colesterol-memória) = mecanismo inesperado

**Padrão 5 — Avatar feminino para produto masculino** (Celtic Salt Trick)
- Anúncios com mulheres apresentando truque de ingrediente para problema de disfunção erétil masculina
- Hipótese: targeting de parceiras/esposas que querem ajudar o marido → menor concorrência de leilão
- **Adaptar:** Para BioRitmo — testar hooks direcionados ao parceiro/família quando aplicável

---

### Nichos em Escala (por volume total de anúncios)

| Nicho | Volume Total | # Offers | Saturação |
|---|---|---|---|
| Emagrecimento | ~65.000 ads | 7 offers | Alta — mas gelatina/sal ainda funcionando |
| Diabetes/Glicose | ~50.000 ads | 5 offers | Crescendo forte — menos saturado que emagrece |
| Alzheimer/Memória | ~29.000 ads | 2 offers | Médio — poucos players grandes |
| Disfunção Erétil | ~50.000 ads | 1 offer | Aparentemente 1 dominante no mercado |
| Visão | ~50.000 ads | 1 offer | Aparentemente 1 dominante |
| Neuropatia | ~16.000 ads | 1 offer | Emergindo — pouca concorrência |
| Fungos | ~9.500 ads | 1 offer | Nicho novo — Escudo de Microplástico |

**Insight-chave:** Diabetes está emergindo como o nicho com mais novos entrantes (5 offers no top 20). Emagrecimento com gelatina/sal ainda escala mas começa a saturar. Neuropatia e Fungos/Microplástico são nichos com white space.

---

### Swipe Cards — Top 5

---
**SWIPE: Celtic Salt Trick - Vigor Peak**
Produto: Vigor Peak
Nicho: Disfunção Erétil
Anúncios: 50.001 | Status: ATIVO | Desde: jan/26
Idioma: Alemão (Deutsch) | Tráfego: Facebook | Formato: VSL

AVATAR: Homens 45-65 com queda de performance sexual (ou parceiras que buscam solução)
AWARENESS LEVEL: 3-4 (sabe do problema, não conhece esse mecanismo)
DRE: Vergonha + desejo de reversão

CRIATIVO OBSERVADO: Mulher em cozinha apresentando ingrediente (sal celta) como "truque secreto"
MUP: Deficiência de mineral específico → queda de testosterona/circulação
MUS: "Celtic Salt Trick" — sal celta como repositor do mineral ausente
ÂNGULO: Mulher revelando truque de cozinha que resolve problema masculino → baixa concorrência de leilão

O QUE ESTÁ FUNCIONANDO: Avatar feminino + ingrediente cotidiano cotidianamente inesperado + mercado alemão (menos saturado que EN/PT)
O QUE ADAPTAR: Hook de "truque de cozinha" + avatar inesperado (parceiro apresentando solução) para nichos masculinos

---
**SWIPE: Reversal Ritual - Glyco Lean**
Produto: Glyco Lean
Nicho: Diabetes/Glicose
Anúncios: 33.112 | Status: ATIVO | Desde: mar/26
Idioma: English | Tráfego: Facebook | Formato: VSL

AVATAR: Diabéticos tipo 2 ou pré-diabéticos frustrados com medicação/dieta
AWARENESS LEVEL: 3-4
DRE: Medo de progressão da doença + frustração com soluções convencionais

MUP: "Glicose travada" — mecanismo que impede que a glicose seja regulada normalmente
MUS: "Reversal Ritual" — protocolo de reversão (não controle) do nível glicêmico
ÂNGULO: Reversão vs. controle — posiciona contra toda a medicina convencional que só "controla"

O QUE ESTÁ FUNCIONANDO: Palavra "Reversal" (reversão) cria esperança onde concorrentes prometem apenas "controle" — upgrades o desejo de nível 3 para nível 4-5
O QUE ADAPTAR: Para BioRitmo — usar "Ressincronização" (ação de reversão) em vez de "regulação" ou "melhora"

---
**SWIPE: GLP-1 Natural com Gelatina - LipoMax**
Produto: LipoMax
Nicho: Emagrecimento
Anúncios: 8.406 | Status: ATIVO | Desde: abr/26
Idioma: English | Tráfego: Facebook | Formato: VSL

AVATAR: Mulheres 35-55 que ouviram falar do Ozempic mas não querem/podem tomar medicamento
AWARENESS LEVEL: 4 (conhece solução farmacêutica, busca alternativa)
DRE: Desejo + raiva com preço/acesso/efeitos colaterais do Ozempic

MUP: "GLP-1 suprimido" — hormônio da saciedade que o corpo para de produzir com a idade
MUS: "GLP-1 Natural com Gelatina" — gelatina estimula produção natural do mesmo hormônio do Ozempic
ÂNGULO: Ozempic natural/gratuito — conecta ao farmacêutico famoso sem usar o nome

O QUE ESTÁ FUNCIONANDO: Aproveita o hype do Ozempic sem mencionar, cria desejo em quem já conhece o resultado esperado
O QUE ADAPTAR: Para BioRitmo — mencionar o que faz sem nomear o farmacêutico equivalente. Ex: "o mesmo mecanismo que os médicos prescrevem, mas via [ingrediente natural]"

---
**SWIPE: Farmacopéia Temperista**
Produto: Low Ticket PT-BR
Nicho: Outros (receitas/temperos)
Anúncios: 7.407 | Status: ATIVO | Desde: jun/25
Idioma: PT-BR | Tráfego: Facebook | Formato: Quiz

AVATAR: Brasileiros interessados em saúde natural, receitas caseiras, temperos medicinais
AWARENESS LEVEL: 2-3
DRE: Curiosidade + desejo de autonomia na saúde

MUS: "Farmacopéia" (palavra médica) + "Temperista" (popular/cotidiano) = contraste que cria curiosidade
ÂNGULO: Quiz funnel em PT-BR para low ticket — raríssimo no top 20 (só 2 PT-BR)

O QUE ESTÁ FUNCIONANDO: Único quiz/low ticket em PT-BR escalando — white space enorme. Palavra "Farmacopéia" empresta credibilidade médica a produto popular
O QUE ADAPTAR: Estrutura de quiz para qualificação antes da oferta principal. Nome com contraste técnico/popular para criar curiosidade imediata

---
**SWIPE: Escudo de Microplástico**
Produto: Nutraceutico antifúngico
Nicho: Fungos/Toxinas
Anúncios: 9.565 | Status: ATIVO | Desde: jan/26
Idioma: English | Tráfego: Facebook | Formato: VSL

AVATAR: Pessoas preocupadas com saúde ambiental, microplásticos, toxinas — avatar mais consciente
AWARENESS LEVEL: 3 (sabe que microplástico é problema, não conhece solução)
DRE: Medo de contaminação invisível + raiva da indústria/meio ambiente

MUP: Microplásticos acumulados no organismo causam disfunção sistêmica
MUS: "Escudo" — protocolo de proteção/remoção de microplásticos
ÂNGULO: Vilão novo (microplástico) = mecanismo não saturado + medo legítimo validado pela mídia

O QUE ESTÁ FUNCIONANDO: Vilão de mídia (microplástico) transformado em MUP específico. Alta emoção + baixa saturação de copy específica para esse vilão
O QUE ADAPTAR: Monitorar se esse vilão se esgota nos próximos 3 meses. Princípio aplicável: usar vilão recente da mídia como MUP antes de saturar

---

### Categorias a Monitorar

**Emagrecimento / Metabolismo Feminino (concorrentes do BioRitmo)**
Monitorar: anúncios para mulheres 35-55, promessas de metabolismo, hormônios, energia, sono
Sinal de escalada: ad no ar há +14 dias com variações múltiplas rodando

**Nutraceuticos Gerais**
Monitorar: suplementos de memória, imunidade, articulações, colesterol
Sinal de escalada: mesmo creative em múltiplos formatos (vídeo + imagem + carrossel)

**Infoprodutos DR**
Monitorar: cursos de renda extra, tráfego, copy, lançamentos
Sinal de escalada: mesmo VSL rodando sem interrupção há +21 dias

---

### Como Usar a Biblioteca de Anúncios Meta
1. Acesse: facebook.com/ads/library
2. País: Brasil | Tipo: Anúncios | Categoria: Todos os anúncios
3. Busque o nome do produto ou marca concorrente
4. Filtro "Anúncios ativos" → ordenar por "Data de início" mais antigas
5. Ads ativos há mais tempo = ads que estão funcionando (não pausados por baixo ROAS)
6. Múltiplas variações do mesmo ad = escalando com budget alto
7. Capturar: hook, primary text, headline, formato — preencher o swipe card

---

## CAMADA 3 — Referências Globais (EN → BR)

### Stefan Georgi
**Mecanismo assinatura:** RMBC II — Research → Mechanism → Brief → Copy
**Leads preferidos:** Segredo/Informação + História para mercados de saúde/nutra
**Onde encontrar swipes:** SwipersPro.com + Stefan Georgi Newsletter
**O que traduz bem para BR:** Estrutura de mecanismo (MUP/MUS), proof density, VSL structure
**O que NÃO traduz:** Referências culturais americanas, humor seco, copy muito direta (BR precisa de mais rapport)

**Swipe Anotada — VSL "The Odd Ice Hack" (emagrecimento)**
- Hook: inversão total de crença ("não é dieta, não é exercício — é temperatura corporal")
- MUP: "baixa temperatura interna" como causa raiz do metabolismo lento
- Prova: estudo de universidade estrangeira como autoridade
- O que aplicar em BR: estrutura de inversão + mecanismo inesperado + origem científica de país neutro

---

### Gary Halbert
**Mecanismo assinatura:** Agitação extrema antes da solução — "a dor precisa ser insuportável antes de apresentar o alívio"
**Leads preferidos:** Histórico de vida pessoal + problema real do autor
**Onde encontrar swipes:** TheGaryHalbertLetter.com (arquivo completo, gratuito)
**O que traduz bem para BR:** Voz íntima e pessoal, agitação visceral, P.S. estratégico
**O que NÃO traduz:** Tom de "cara durão", referências de anos 80, linguagem muito masculina para avatares femininos

**Swipe Anotada — "The Coat of Arms Letter"**
- Mecanismo: identidade genealógica + exclusividade de produto personalizado
- Hook: curiosidade sobre sobrenome do prospect
- O que aplicar em BR: personalização como hook + identidade como motor de desejo

---

### Dan Kennedy
**Mecanismo assinatura:** Magnetic Marketing — atrair prospect certo em vez de perseguir qualquer um
**Leads preferidos:** Proclamação direta para audiência qualificada + promessa específica com número
**Onde encontrar swipes:** DanKennedy.com + GKIC newsletter archives + grupos de copy EN
**O que traduz bem para BR:** Qualificação agressiva no lead, ofertas com stack de valor, garantia como argumento
**O que NÃO traduz:** Tom muito agressivo/confrontacional (BR prefere tom de "amigo especialista")

**Princípio adaptável:** "Diga para quem NÃO é antes de dizer para quem É" — qualificação negativa cria desejo nos que ficam

---

### Gary Bencivenga
**Mecanismo assinatura:** Proof-first copywriting — a prova precede qualquer promessa
**Leads preferidos:** Prova científica → mecanismo → produto
**Onde encontrar swipes:** BencivengaBullets.com + Marketing Bullets newsletter
**O que traduz bem para BR:** Hierarquia de prova, credenciais antes de vender, copy para mercados céticos
**O que NÃO traduz:** Escrita mais lenta e deliberada — BR responde melhor a ritmo mais rápido

**Princípio adaptável:** Em mercados saturados, a prova substitui a promessa como lead. Quando todos prometem X, prove X antes de prometer.

---

### John Carlton
**Mecanismo assinatura:** Agora do fracasso → descoberta → resultado — jornada em 3 atos
**Leads preferidos:** História de personagem com transformação específica
**Onde encontrar swipes:** SimpleWritingSystem.com + Marketing Rebel archives
**O que traduz bem para BR:** Estrutura de história com tensão real, linguagem de rua (não corporativa), absolvição do prospect
**O que NÃO traduz:** Referências de esportes americanos, machismo implícito em copies antigas

**Princípio adaptável:** "One-legged golfer" — personagem com handicap improvável + resultado inesperado = credibilidade máxima

---

## Protocolo Semanal de Atualização

**Todo domingo — 15 minutos:**
1. Abrir Biblioteca Meta → buscar 3 concorrentes diretos
2. Identificar ads novos ativos (não estavam na semana anterior)
3. Para cada ad interessante: preencher swipe card e adicionar aqui
4. Marcar ads que saíram do ar como ~~INATIVO~~

**Quando um concorrente lança algo novo:**
1. Capturar imediatamente (ads mudam rápido)
2. Analisar: o que é novo nesse ângulo? O que eu já tenho que se relaciona?
3. Hipótese: por que está funcionando?
4. Decisão: adaptar o eixo agora ou monitorar por 14 dias antes?

**Regra de ouro:**
> Quando um concorrente escala um ângulo novo — copie o **eixo**, nunca o texto.
> Texto copiado = sem diferenciação. Eixo adaptado = ombros de gigante.
